...
var BarChart = function() {
    // Default values, variables that should be accessible within the namespace
    ...
    var chart = function(selection) {
        ...
        selection.each(function(data) {
            ...
            var gEnter = svg.enter().append("svg").attr('height', height)
                .attr('width', width).attr('transform', 'translate(' + movement + ", 0)").append('g')
            ...
            // Set ordinal X scale
            var xScale = d3.scaleBand().range([0, innerWidth]).padding(.2).domain(xValues);
            ...
            yScale = d3.scaleLinear().range([innerHeight, 0]).domain([0, yMax]);
            ...
            bars.transition()
                ...
                .attr('height', function(d) {
                    return innerHeight - yScale(d[yVar])
                })
